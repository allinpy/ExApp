# based on https://github.com/pyqtgraph/pyqtgraph/blob/master/pyqtgraph/examples/ExampleApp.py
import keyword
import os
import pkgutil
import re
...
class ExampleLoader(QtWidgets.QMainWindow):
    ...
    def __init__(self, dir_path):
        ...
        self.itemCache = []
        self.populateTree(self.ui.exampleTree.invisibleRootItem(), utils.list_py_files(self.dir_path))
    ...
    def filterByContent(self, text=None):
        ...
        if not validRegex:
            return
        checkDict = unnestedDict(utils.list_py_files(self.dir_path))
    ...
    def getMatchingTitles(self, text, exDict=None, acceptAll=False):
        if exDict is None:
            exDict = utils.list_py_files(self.dir_path)
        text = text.lower()
        titles = []
        for kk, vv in exDict.items():
